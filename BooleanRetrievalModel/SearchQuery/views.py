...
def renderHome(request):
    ...
    global sizes_of_stemmed_lists        
    query_dictionary = {}        
    term_document_dictionary = {}    
    list_of_documentID = list()    
    sizes_of_stemmed_lists = list()
    query = request.POST
    if bool(query):
        print(query)
        ...
        print("Query: ",q)
        list_here = []
        list_here = processingQuery(q)
        print(list_here)
        context = {
            'list_of_documents': list_here
        }
    return render(request, 'index.html',context)
    # return HttpResponse("Home Page loaded")
